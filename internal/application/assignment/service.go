...
func (s Service) Accept(ctx context.Context, proposalID, unitID, actorID string) error {
	return s.Tx.WithinTransaction(ctx, func(txCtx context.Context) error {
		...
		if err := a.Accept(unitID, actorID, acceptedAt); err != nil {
			return err
		}
		...
		version := p.Version
		if receipt.ProposalID != p.ID {
			return shared.ErrConflict
		}
		if err := p.Advance(proposal.StatusAssigned, proposal.StatusAccepted, acceptedAt); err != nil {
			return err
		}
