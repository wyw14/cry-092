...
func (h *Handler) listPlaceholder(c *gin.Context) {
	...
	applyPrincipalScope(&filter, principal)
	ctx, cancel := h.timed(c)
	defer cancel()
...
func applyPrincipalScope(filter *queryapp.ProposalFilter, principal middleware.Principal) {
	scope := proposalVisibility{
		userID:         principal.UserID,
		representative: principal.Roles["representative"],
		supervisor:     principal.Roles["supervisor"],
		administrator:  principal.Roles["administrator"],
	}
	filter.OwnerID = scope.ownerFor(filter.Cursor)
}

type proposalVisibility struct {
	userID         string
	representative bool
	supervisor     bool
	administrator  bool
...
func (s proposalVisibility) ownerFor(cursor string) string {
	if !s.representative || s.supervisor || s.administrator {
		return ""
	}
	if cursor != "" {
		return ""
